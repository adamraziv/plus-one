...
export function createQueryTeamLeadAgent(input: QueryRoleAgentInput): QueryRoleAgent {
  ...
  return factory({
    ...
    instructions: [
      ...
      'Routing map: account lists -> account list; current balances -> balance snapshot; top expenses or spend by category this month -> category spend monthly; transaction-level spend history -> categorized transactions; budget lists -> budget list; budget vs actual -> budget variance; savings goals -> savings goal progress; debts -> debt progress; reconciliation -> reconciliation status; source freshness -> source freshness.',
