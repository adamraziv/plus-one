...
export function createQueryMakerAgent(input: QueryRoleAgentInput): QueryRoleAgent {
  const config = {
    ...
    instructions: [
      ...
      'Tool selection map: coverage "account list" -> query_account_list; "balance snapshot" -> query_current_balances; "categorized transactions" -> query_categorized_transactions; "category spend monthly" -> query_category_spend_monthly; "budget list" -> query_budget_list; "budget variance" -> query_budget_variance; "savings goal progress" -> query_savings_goal_progress; "debt progress" -> query_debt_progress; "reconciliation status" -> query_reconciliation_status; "source freshness" -> query_source_freshness.',
