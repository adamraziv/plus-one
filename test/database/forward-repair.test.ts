...
describe('database forward repair', () => {
  it('restores role privileges, ownership, and verification after privilege drift', async () => {
    ...
    try {
      ...
      await admin.query('ALTER TABLE operations.households OWNER TO plus_one_migrator');
      ...
      await expect(admin.query(verifySql)).rejects.toMatchObject({ code: 'P0001' });

      await admin.query(repairSql);

      await expect(admin.query(verifySql)).resolves.toBeDefined();
