...
describe('canonical artifacts', () => {
  it('canonicalizes object keys independently of insertion order', () => {
    expect(canonicalizeJson({ b: 2, a: { d: true, c: null } })).toBe(
      '{"a":{"c":null,"d":true},"b":2}',
    );
  ...
  it('rejects values outside the JSON contract', () => {
    expect(() => canonicalizeJson({ amount: 1n } as never)).toThrow(/JSON value/);
    expect(() => canonicalizeJson({ invalid: Number.NaN } as never)).toThrow(/JSON value/);
  });
});
