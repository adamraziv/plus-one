...
import { ArtifactEnvelopeSchemaV1, CheckerVerdictSchemaV1, RuntimePolicySchemaV1, TaskStatusSchemaV1, } from './index.js';
describe('operational contracts', () => {
    it('requires versioned immutable artifact identity and lowercase sha256', () => {
        ...
        expect(ArtifactEnvelopeSchemaV1.safeParse({
            ...parsed,
            artifactHash: 'A'.repeat(64),
        }).success).toBe(false);
    });
    it('requires checker coverage of an exact artifact ID and hash', () => {
        expect(CheckerVerdictSchemaV1.safeParse({
            verdict: 'accepted',
    ...
    it('fixes lifecycle and bounded runtime policy vocabulary', () => {
        ...
        expect(RuntimePolicySchemaV1.parse({
